    ...
    $(function () {
        ...
        $.entwine("ss.frontEndAdmin", function ($) {
            $("div.admin-panel").entwine({
                ...
                ToolsWrap: $("<div>").addClass("admin-tools-wrap"),
                ...
                ToolsUserWrap: $("<div>" + frontEndAdmin.currentUser + "</div>").addClass("admin-panel-user"),
                ToolsLogoutLink: $("<a class='logout-link'>" + frontEndAdmin.logoutLink + "</a>").attr("href", '/Security/logout').attr("target", "_top").addClass("admin-tools-logoutlink admin-tools-link btn btn-default"),
                onmatch: function () {
                    if (!$("body").hasClass("cms-dialog")) {
                        ...
                        self
                                ...
                                .append(
                                        ...
                                        .append(
                                                self.getAdminBtn().css({marginBottom : "80px"})
                                        		)
                                        .append(
                                                self.getToolsEditModeLabel().attr("for", self.getToolsEditMode().attr("id")).css({marginTop: "15px"})
                                                )
                                        .append(
                                                self.getToolsEditMode()
                                                )
                                        .append(
                                                self.getToolsHighlightLabel().attr("for", self.getToolsHighlight().attr("id")).css({marginTop:"5px"})
                                                )
                                        .append(
                                                self.getToolsHighlight()
                                                )
                                        .append(
                                                self.getToolsUserWrap().css({margin:"5px", marginTop:"15px"})
                                        		)
                                        .append(
                                                self.getToolsLogoutLink()
                                          		)
                                        );
                        ...
                        if (!isNaN(settings.y)) {
                            ...
                            if( settings.y < 0 || settings.y > maxY-75) settings.y = 10; 
            ...
            $("#adminToggle").entwine({
                onmatch: function () {
                    this.button({icons: {primary: "ui-icon-gear", secondary: frontEndAdmin.toggleIconClosed}});
                },
                onclick: function () {
                    $("div.admin-panel").toggleAdmin();
                },
                setIcon: function (icon) {
                    this.button({icons: {primary: "ui-icon-gear", secondary: icon}});
                }
            });
            ...
            $(".admin-tools-highlight").entwine({
                onmatch: function () {
                    this.button({icons: {primary: "ui-icon-flag", secondary: "ui-icon-circle-close"}});
                    this._super();
                },
                onchange: function () {
            ...
            $(".admin-tools-logoutlink").entwine({
                onmatch: function () {
                    this.button({icons: {primary: "ui-icon-power"}});
                },
//                onclick: function () {
//                }
            });
